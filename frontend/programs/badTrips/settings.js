...
const createHeader = (tableDom) => {
  ...
  const thBicycle = components.getTagTH("Разрешено использование велосипеда?", "Отметьте пиццерию в которой разрешено курьерам использовать велосипед для доставки. Обратите внимание что если в пиццерии не указано что возможно использовать велосипед то при выборе курьеров велосипеда данных заказ попадет в проблемные независимо от просрочки")
  theadTrDom.append(thBicycle);

  const thLongOnFoot = components.getTagTH("Контролируем дальние походы?", "Если вы выбираете контроль дальних походов пешком то укажите максимальное плановое вермя. Это сделано чтобы исключить махинации курьерами по выбору типа транспортного средства. По умолчанию плановое время 14 минут, это где то расстояние пешком около 1,5 км от пиццерии")
