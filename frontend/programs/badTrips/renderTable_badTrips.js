...
export async function renderTable(
  ...
  if (!filterToCourier) {
    arrayData.sort(
      (a, b) =>
        new Date(a.handedOverToDeliveryAt) - new Date(b.handedOverToDeliveryAt)
    );
  }
  ...
  const captionEl = components.getTagCaption(
    "Программа контроля за проблемными поездками курьеров"
  );
  ...
  let countDelays = arrayData.filter(
  ...
  countDelays = arrayData.filter(
    (el) => el.directorComment.content === "Просрочка"
  ).length;
  if (countDelays) {
    const spanEl = components.getTagSpan_badge(countDelays);
    spanEl.textContent = countDelays;
  ...
  arrayData.forEach((order) => {
    ...
    let time = components.getTagTD(
      new Date(order.handedOverToDeliveryAt).toLocaleString().slice(0, 17)
    );
    trEl.append(time);
    let fio = components.getTagTD(order.fio);
    trEl.append(fio);

    let sertificate = order.wasLateDeliveryVoucherGiven
      ? `<b>Выдан</b>`
      : "Нет";
    ...
    let titleH1 = components.getTagH(
      1,
      "Подробная информация о поездке курьера"
    );
    ...
    let dateResponceCourier = order.dateResponceCourier
      ? order.dateResponceCourier
      : "Нет ответа от курьера";
    modalBody.innerHTML = `
    <b>Общие данные</b><br>
...
Если курьер отжимает кнопку поехали не в курьерской, то это считется фальсификацией данных">начала</a>: поездки ${new Date(
      ...
      .toLocaleString()
      .slice(0, 17)}<br>
    Время окончания поездки: ${new Date(order.orderFulfilmentFlagAt)
      .toLocaleString()
      .slice(0, 17)}<br>
...
`;
    ...
    if (order.expiration >= 10 && order.expiration < 20)
  ...
  liTimes.forEach((el) => {
    el.addEventListener("click", () =>
      filter.filterToDate(el.value, fullDataUnit)
    );
  });
