...
const generateTBody = (response) => {
    ...
    response.items.forEach((item) => {
        ...
        cancelResolutionHRSelect.disabled = role !== 'менеджер офиса'
        const cancelResolutionHRTrueOpt = components.getTagOption('Согласиться с решением управляющего', true)
        cancelResolutionHRTrueOpt.selected = item.cancel_resolution_hr === true
        const cancelResolutionHRFalseOpt = components.getTagOption('Отменить решение управляющего', false)
        cancelResolutionHRFalseOpt.selected = item.cancel_resolution_hr === false
