    ...
    if (e.target.textContent === "Редактировать подразделение") {
      ...
      if (unit.type === "Пиццерия") {
        ...
        function timeWorkDelivery() {
          ...
          const startInput = components.getInputEl("", "time", unit.timeWork.delivery.workingTimeStart, "старт");
          divElStart.append(startInput);

          let divElStop = components.getDivEl("col-auto");
          divElStop.classList.add("mb-3");
          const stopInput = components.getInputEl("", "time", unit.timeWork.delivery.workingTimeStop, "стоп");
        ...
        function timeWorkRestoran() {
          ...
          let divElStart = components.getDivEl("col-auto");
          const startInput = components.getInputEl("", "time", unit.timeWork.restoran.workingTimeStart, "старт");
          divElStart.append(startInput);

          let divElStop = components.getDivEl("col-auto");
          const stopInput = components.getInputEl("", "time", unit.timeWork.restoran.workingTimeStop, "стоп");
          divElStop.append(stopInput);
          divElDel.append(divElStart, divElStop);
        }
        timeWorkDelivery();
